...
class Arr
{
     ...
     * @return array
     */
    public function tree($data, $title, $fieldPri = 'cid', $fieldPid = 'p_id')
    {
    ...
    public function isChild(
        ...
        $_data = $this->channelList($data, $p_id, '', $fieldPri, $fieldPid);
        foreach ($_data as $c) {
            //目标栏目为源栏目的子栏目
            if ($c[$fieldPri] == $sid) {
                return true;
     ...
     * @return bool
     */
    public function hasChild($data, $cid, $fieldPid = 'p_id')
    {
        foreach ($data as $d) {
            if ($d[$fieldPid] == $cid) {
                return true;
